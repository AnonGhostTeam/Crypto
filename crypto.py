...
parser.add_argument('--login', action='store_true', help='login your instagram account[REQUIRED]')
parser.add_argument('--change', action='store_true', help='change your username and password')
        ...
        if args.number:
            ...
            if private == False:
                ...
                if ask_like_post.lower() == 'y':
                    ...
                    while True:
                        url_post = input(
                            '[!] Please copy the shortlink [https://www.instagram.com/p/(JUST COPY THE SHORTLINK)/]: ')
            ...
            if check_story == True:
                download_stories = input(
                    f'[!] The username {username} already have stories. Do you want to downloads?[y/n]: ')
            ...
            download_highlights = input(
                f'[!] If you see a highlights on username {username} you can download it. Are you see a highlights or want to download?[y/n]: ')
            ...
            if private == False:
                ...
                if downloads.lower() == 'y':
                    ...
                    print('[!] You can check in directory ', os.getcwd(),
                          'from all results web scraping if you do download higlights or other(s)')
                    print('[+] Success for scraping')
                else:
                    self.Asking_else()

        elif args.login:
            username_create = open("usernames.txt", "a")
            username_create.write(args.username)
            username_create.close()

            password_create = open("passwords.txt", "a")
            password_create.write(args.password)
            password_create.close()
            print('[+] Your username and password has been stored in : ', os.getcwd())

        elif args.change:
            username_change = open('usernames.txt', 'w')
            username_change.write(args.username)
            username_change.close()

            password_change = open('passwords.txt', 'w')
...
if __name__ == "__main__":
    ...
    # handling error
        ...
        # this mean for clear screen on mac and linux
            ...
            system('clear')
        subprocess.run([sys.executable, 'crypto.py', '-h'])
